...
if (loginForm) {
    loginForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(`加载users.json失败: ${response.status}`);
            }
...
function renderAdsList() {
    ...
    data.ads.forEach(ad => {
        ...
        tr.innerHTML = `
            ...
            <td style="padding: 12px;">${getPositionText(ad.position)}</td>
            <td style="padding: 12px; text-align: center;">
                <span style="padding: 4px 12px; border-radius: 12px; font-size: 12px; ${ad.status === 'active' ? 'background-color: #d1fae5; color: #065f46;' : 'background-color: #fef2f2; color: #991b1b;'}">
...
function editAd(adId) {
    ...
    if (ad) {
        ...
        const adImagePreview = document.getElementById('ad-image-preview');
        if (ad.image) {
            adImagePreview.innerHTML = `<img src="${ad.image}" style="width: 120px; height: 80px; object-fit: cover; border-radius: 4px;">`;
        } else {
            adImagePreview.innerHTML = '';
        }
...
if (adImageInput) {
    adImageInput.addEventListener('change', function(e) {
        ...
        if (file) {
            ...
            reader.onload = function(e) {
                const adImagePreview = document.getElementById('ad-image-preview');
                adImagePreview.innerHTML = `<img src="${e.target.result}" style="width: 120px; height: 80px; object-fit: cover; border-radius: 4px;">`;
            };
            reader.readAsDataURL(file);
        } else {
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        console.log('数据初始化完成，数据:', data);
        
        // 检查数据结构
        console.log('分类数据:', data.categories);
        console.log('工具数据:', data.tools);
        
        // 数据初始化完成后再渲染
        console.log('开始渲染分类和工具...');
        loadCategoriesToSelect();
...
function refreshFrontendPage() {
    ...
    if (frontendWindow) {
        ...
        frontendWindow.location.reload();
    } else {
        alert('请允许弹出窗口，或手动刷新前端页面查看效果');
    }
}
...
function setupDataExport() {
    ...
    if (syncDataBtn) {
        syncDataBtn.addEventListener('click', async function() {
            console.log('同步数据到数据库...');
            try {
                const data = getData();
                
                // 导出各个模块的数据到JSON文件
                downloadFile(JSON.stringify({ categories: data.categories }, null, 2), 'categories.json', 'application/json');
                downloadFile(JSON.stringify({ tools: data.tools }, null, 2), 'tools.json', 'application/json');
                downloadFile(JSON.stringify({ setting: data.setting }, null, 2), 'setting.json', 'application/json');
                downloadFile(JSON.stringify({ pages: data.pages }, null, 2), 'pages.json', 'application/json');
                downloadFile(JSON.stringify({ users: data.users }, null, 2), 'users.json', 'application/json');
                downloadFile(JSON.stringify({ ads: data.ads }, null, 2), 'ads.json', 'application/json');
                
                alert('数据库文件下载成功！请将下载的文件覆盖到项目的data文件夹中。');
            } catch (err) {
    ...
    if (exportCategoriesBtn) {
        exportCategoriesBtn.addEventListener('click', function() {
            const data = getData();
            const jsonContent = JSON.stringify(data.categories, null, 2);
    ...
    // 导出工具数据
    const exportToolsBtn = document.getElementById('export-tools-btn');
    if (exportToolsBtn) {
        exportToolsBtn.addEventListener('click', function() {
            const data = getData();
            const jsonContent = JSON.stringify(data.tools, null, 2);
            downloadFile(jsonContent, 'tools.json', 'application/json');
            alert('工具数据导出成功！');
        });
    }
    ...
    if (exportHomeBtn) {
        exportHomeBtn.addEventListener('click', function() {
            const data = getData();
            const jsonContent = JSON.stringify(data.setting, null, 2);
            downloadFile(jsonContent, 'setting.json', 'application/json');
            alert('网站设置导出成功！');
        });
    }
    
    // 导出页脚设置（已合并到网站设置中）
    const exportFooterBtn = document.getElementById('export-footer-btn');
    if (exportFooterBtn) {
        exportFooterBtn.addEventListener('click', function() {
            const data = getData();
            const jsonContent = JSON.stringify(data.setting, null, 2);
            downloadFile(jsonContent, 'setting.json', 'application/json');
            alert('页脚设置已合并到网站设置中，导出成功！');
        });
    }
    ...
    if (exportPagesBtn) {
        exportPagesBtn.addEventListener('click', function() {
            const data = getData();
            const jsonContent = JSON.stringify(data.pages, null, 2);
            downloadFile(jsonContent, 'pages.json', 'application/json');
            alert('页面数据导出成功！');
        });
    }
    ...
    // 导出广告数据
    const exportAdsBtn = document.getElementById('export-ads-btn');
    if (exportAdsBtn) {
        exportAdsBtn.addEventListener('click', function() {
            const data = getData();
            const jsonContent = JSON.stringify(data.ads, null, 2);
            downloadFile(jsonContent, 'ads.json', 'application/json');
            alert('广告数据导出成功！');
        });
    }
}
...
function setupFormSubmissions() {
    ...
    if (homeForm) {
        homeForm.addEventListener('submit', function(e) {
            ...
            if (homeLogo.files && homeLogo.files[0]) {
                ...
                reader.onload = function(e) {
                    ...
                    if (homeCustomTheme.files && homeCustomTheme.files[0]) {
                        ...
                        themeReader.onload = function(themeE) {
                            data.setting.customTheme = themeE.target.result;
                            saveData(data);
                ...
                reader.readAsDataURL(homeLogo.files[0]);
            } else {
                // 没有上传新logo，使用现有logo
                data.setting = {
                    title: homeTitle,
        ...
        addCategoryForm.addEventListener('submit', function(e) {
            ...
            if (categoryId) {
                ...
                if (categoryIndex !== -1) {
                    ...
                    if (categorySuccess) {
                        categorySuccess.textContent = '分类编辑成功！';
                        categorySuccess.style.display = 'block';
                    }
                }
            } else {
                // 添加分类，生成连续的ID
                const maxId = data.categories.length > 0 ? Math.max(...data.categories.map(c => c.id)) : 0;
                const newCategory = {
                    id: maxId + 1,
                    name: categoryName
                };
                ...
                const categorySuccess = document.getElementById('category-success');
                if (categorySuccess) {
                    categorySuccess.textContent = '分类添加成功！';
                    categorySuccess.style.display = 'block';
                }
            }
    ...
    if (addToolForm) {
        addToolForm.addEventListener('submit', function(e) {
            ...
            if (iconUploadType === 'file') {
                const fileInput = document.getElementById('tool-icon');
                if (fileInput.files && fileInput.files[0]) {
                    // 简单的Base64编码模拟上传
                    const reader = new FileReader();
                    reader.onload = function(e) {
                        toolIcon = e.target.result;
                        saveToolData(toolId, toolCategory, toolName, toolDescription, toolUrl, toolIcon);
                    };
                    reader.readAsDataURL(fileInput.files[0]);
                    return;
                }
            } else {
                toolIcon = document.getElementById('tool-icon-url').value;
            }
        ...
        addAdForm.addEventListener('submit', function(e) {
            ...
            if (fileInput.files && fileInput.files[0]) {
                ...
                reader.onload = function(e) {
                    ...
                    saveAdData(adId, adName, adUrl, adImage, adPosition, adStatus);
                };
                reader.readAsDataURL(fileInput.files[0]);
                return; // 防止后续代码继续执行
            }
    ...
    if (addUserForm) {
        addUserForm.addEventListener('submit', function(e) {
            ...
            saveUserData(userId, userUsername, userPassword, userRole);
        });
    }
    ...
    if (addPageForm) {
        addPageForm.addEventListener('submit', function(e) {
            ...
            savePageData(pageId, pageTitle, pageSlug, pageContent, pageStatus);
        });
    }
}

// 保存工具数据
function saveToolData(toolId, toolCategory, toolName, toolDescription, toolUrl, toolIcon) {
    const data = getData();
    ...
    if (toolId) {
        ...
        if (toolIndex !== -1) {
            ...
            if (toolSuccess) {
                toolSuccess.textContent = '工具编辑成功！';
                toolSuccess.style.display = 'block';
            }
        }
    } else {
        // 添加工具，生成连续的ID
        const maxId = data.tools.length > 0 ? Math.max(...data.tools.map(t => t.id)) : 0;
        const newTool = {
            id: maxId + 1,
            category_id: parseInt(toolCategory),
            name: toolName,
        ...
        const toolSuccess = document.getElementById('tool-success');
        if (toolSuccess) {
            toolSuccess.textContent = '工具添加成功！';
            toolSuccess.style.display = 'block';
        }
    }
...
// 保存广告数据
function saveAdData(adId, adName, adUrl, adImage, adPosition, adStatus) {
    const data = getData();
    ...
    if (adId) {
        ...
        if (adIndex !== -1) {
            ...
            const adSuccess = document.getElementById('ad-success');
            if (adSuccess) {
                adSuccess.textContent = '广告编辑成功！';
                adSuccess.style.display = 'block';
            }
        }
    } else {
        // 添加广告，生成连续的ID
        const maxId = data.ads.length > 0 ? Math.max(...data.ads.map(a => a.id)) : 0;
        const newAd = {
            id: maxId + 1,
            name: adName,
            url: adUrl,
    ...
    setTimeout(() => {
        const adSuccess = document.getElementById('ad-success');
        if (adSuccess) {
            adSuccess.style.display = 'none';
        }
    }, 3000);
}

// 保存用户数据
function saveUserData(userId, userUsername, userPassword, userRole) {
    const data = getData();
    ...
    if (userId) {
        ...
        if (userIndex !== -1) {
            ...
            if (userSuccess) {
                userSuccess.textContent = '用户编辑成功！';
        ...
        const userSuccess = document.getElementById('user-success');
        if (userSuccess) {
            userSuccess.textContent = '用户添加成功！';
            userSuccess.style.display = 'block';
        }
    }
...
// 保存页面数据
function savePageData(pageId, pageTitle, pageSlug, pageContent, pageStatus) {
    const data = getData();
    ...
    if (pageId) {
        ...
        if (pageIndex !== -1) {
            data.pages[pageIndex] = {
                ...
                content: pageContent,
                status: pageStatus,
                created_at: data.pages[pageIndex].created_at,
            ...
            if (pageSuccess) {
                pageSuccess.textContent = '页面编辑成功！';
        ...
        // 添加页面，生成连续的ID
        const maxId = data.pages.length > 0 ? Math.max(...data.pages.map(p => p.id)) : 0;
        const newPage = {
            id: maxId + 1,
            title: pageTitle,
            slug: pageSlug,
        ...
        const pageSuccess = document.getElementById('page-success');
        if (pageSuccess) {
            pageSuccess.textContent = '页面添加成功！';
            pageSuccess.style.display = 'block';
...
function renderPagesList() {
    ...
    thead.innerHTML = `
        <tr style="background-color: #f8fafc; border-bottom: 2px solid #e2e8f0;">
            ...
            <th style="padding: 12px; text-align: left;">页面别名</th>
            <th style="padding: 12px; text-align: center;">状态</th>
            <th style="padding: 12px; text-align: center;">操作</th>
        </tr>
    `;
    ...
    data.pages.forEach(page => {
        ...
        tr.innerHTML = `
            ...
            <td style="padding: 12px; text-align: center;">
                <span style="padding: 4px 12px; border-radius: 12px; font-size: 12px; ${page.status === 'published' ? 'background-color: #d1fae5; color: #065f46;' : 'background-color: #fef2f2; color: #991b1b;'}">
                    ${page.status === 'published' ? '发布' : '草稿'}
                </span>
            </td>
            <td style="padding: 12px; text-align: center;">
